...
class GPTConfig:
    ...
    use_einsum: bool = (
        False  # Whether to ues Einstein summation or jnp.matmul for matrix multiplication in self attention calculation
    )
...
class GPT(nn.Module):
    ...
    def __call__(
        ...
        if targets is not None:
            # (b, t, vocab_size); implements weight tying (https://github.com/google/flax/discussions/2186)
            logits = self.wte.attend(x)
            loss = optax.softmax_cross_entropy_with_integer_labels(
                policy.cast_to_reduce_ops(logits), targets
            ).mean()
            if not train:
                return logits, loss
    ...
    # TODO: imlement model sugery method so that it can fit smaller block
    # sizes.
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        for i in jnp.arange(config.n_layer):
            ...
            params["params"]["h_" + str(i)]["mlp"]["Dense_1"]["bias"] = model_hf.params[
                "transformer"
            ]["h"][str(i)]["mlp"]["c_proj"]["bias"]
            params["params"]["h_" + str(i)]["mlp"]["Dense_0"]["kernel"] = (
                model_hf.params["transformer"]["h"][str(i)]["mlp"]["c_fc"]["kernel"].T
            )
            params["params"]["h_" + str(i)]["mlp"]["Dense_1"]["kernel"] = (
                model_hf.params["transformer"]["h"][str(i)]["mlp"]["c_proj"]["kernel"].T
            )
            params["params"]["h_" + str(i)]["attn"]["c_attn"]["bias"] = model_hf.params[
                "transformer"
            ]["h"][str(i)]["attn"]["c_attn"]["bias"]
            params["params"]["h_" + str(i)]["attn"]["c_proj"]["bias"] = model_hf.params[
                "transformer"
            ]["h"][str(i)]["attn"]["c_proj"]["bias"]
            params["params"]["h_" + str(i)]["attn"]["c_attn"]["kernel"] = (
                model_hf.params["transformer"]["h"][str(i)]["attn"]["c_attn"][
                    "kernel"
                ].T
            )
            params["params"]["h_" + str(i)]["attn"]["c_proj"]["kernel"] = (
                model_hf.params["transformer"]["h"][str(i)]["attn"]["c_proj"][
                    "kernel"
                ].T
            )
        return model, params
