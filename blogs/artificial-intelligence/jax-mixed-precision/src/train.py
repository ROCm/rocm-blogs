...
amp = True # if True, will use mixed precision training (fp32 optimizer, fp16 computate)
skip_infinite = True # skip iterations with infinite grads in mixed precision training
# define initial loss scale
loss_scale = jmp.DynamicLossScale(jnp.asarray(2.0**15, dtype=jnp.float32)) if amp else jmp.NoOpLossScale()
# loss_scale = jmp.NoOpLossScale() # drop in replacement. It does nothing (apart from implement the jmp.LossScale API)
# -----------------------------------------------------------------------------
...
def init_train_state(model, params, learning_rate, weight_decay=None, beta1=None, beta2=None, decay_lr=True, warmup_iters=None, 
                     lr_decay_iters=None, min_lr=None, loss_scale=None) -> TrainState:
...
if init_from == 'scratch':
    ...
    model_args_ckpt, iter_num_ckpt, best_val_loss_ckpt, _, config_ckpt, loss_scale_value, loss_scale_counter = restored_ckpt['metadata']
    loss_scale = jmp.DynamicLossScale(loss_scale=jnp.asarray(loss_scale_value, dtype=jnp.float32), counter=jnp.asarray(loss_scale_counter, dtype=jnp.int32))
...
state, lr_schedule = init_train_state(model, params['params'], learning_rate, weight_decay, beta1, beta2, decay_lr, warmup_iters, 
                     lr_decay_iters, min_lr, loss_scale=loss_scale)
if init_from == "resume":
    with ocp.CheckpointManager(check_path, options=check_options, item_names=('state', 'metadata')) as mngr:
        state = mngr.restore(mngr.latest_step(), args=ocp.args.Composite(state=ocp.args.StandardRestore(state)))['state']

@partial(jax.jit, static_argnames=('train', 'policy'))
def loss_fn(params, x, targets=None, train=True, rng=jax.random.key(0), loss_scale=None, policy=None):
    _, loss = state.apply_fn({'params': params}, x, targets=targets, train=train, rng=rng, loss_scale=loss_scale, policy=policy)
    return loss

@partial(jax.jit, donate_argnums=(0,))
def train_step(state: TrainState, batch: jnp.ndarray, loss_scale: jmp.LossScale=None, rng: jnp.ndarray=jax.random.key(0)):
    x, y = batch
    ...
    return state, loss, key1, loss_scale

def estimate_loss(state, loss_scale=None):
    result = {}
    for split in ['train', 'val']:
        loss = np.zeros(eval_iters)
        for i in range(eval_iters):
            x, y = get_batch(split)
            loss[i] = loss_fn(state.params, x, targets=y, train=False, loss_scale=loss_scale, policy=policy)
...
while True:
    ...
    if iter_num % eval_interval == 0:
        ...
        losses = estimate_loss(state, loss_scale=loss_scale)
                ...
                with ocp.CheckpointManager(check_path, options=check_options, item_names=('state', 'metadata')) as mngr:
                    mngr.save(
                        ...
                        args=ocp.args.Composite(
                            ...
                            metadata=ocp.args.JsonSave((model_args, iter_num, best_val_loss, losses['val'].item(), config, loss_scale.loss_scale.item(), loss_scale.counter.item()))))
            
    if iter_num == 0 and eval_only:
        break

    state, loss, rng_key, loss_scale = train_step(state, get_batch('train'), loss_scale=loss_scale, rng=rng_key)
