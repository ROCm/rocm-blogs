# Originally written by Andrej Karpathy in PyTorch; adapted by Douglas Jia
# to JAX.
"""
...
amp = (
    # if True, will use mixed precision training (fp32 optimizer, fp16
    # computate)
    True
)
...
loss_scale = (
    jmp.DynamicLossScale(jnp.asarray(2.0**15, dtype=jnp.float32))
    if amp
...
# init these up here, can override if init_from='resume' (i.e. from a
# checkpoint)
iter_num = 0
best_val_loss = 1e9
...
if init_from == "scratch":
    ...
    (
        ...
        loss_scale_value,
        loss_scale_counter,
    ) = restored_ckpt["metadata"]
    loss_scale = jmp.DynamicLossScale(
        loss_scale=jnp.asarray(loss_scale_value, dtype=jnp.float32),
        counter=jnp.asarray(loss_scale_counter, dtype=jnp.int32),
    )
...
while True:
    ...
    if iter_num % eval_interval == 0:
        ...
        print(
            f"step {iter_num}: train loss {
                losses['train']:.4f}, val loss {
                losses['val']:.4f}; best val loss to now: {
                min(
                    losses['val'].item(),
                    best_val_loss):.4f}"
        )
    ...
    state, loss, rng_key, loss_scale = train_step(
        state, get_batch("train"), loss_scale=loss_scale, rng=rng_key
    )
