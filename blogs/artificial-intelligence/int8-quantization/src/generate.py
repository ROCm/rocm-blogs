...
# Experimental feature to reduce compilation times, will be on by default
# in future
...
def main(
    ...
    for i in range(start, (num_samples + 10)):
        ...
        memory_bw = model_size * tokens_sec / 1e9
        aggregate_metrics["memory_bw"].append(memory_bw)
        print(
            f"Time for inference {
                i +
                1}: {
                t:.02f} sec total, {
                tokens_sec:.02f} tokens/sec"
        )
        print(f"Bandwidth achieved: {memory_bw:.02f} GB/s")
    print("==========")
    print("==========")
    ...
    if is_speculative:
        ...
        print(
            f"Mean Accepted: {sum([idx *
                                   i for idx, i in enumerate(counts_aggregated)]) /
                              sum(counts_aggregated)}"
        )

    print(
        f"Average tokens/sec of last 10 iterations: {torch.mean(torch.tensor(aggregate_metrics['tokens_per_sec'][-10:])).item():.2f}"
    )
    print(
        f"Std. dev. tokens/sec of last 10 iterations: {torch.std(torch.tensor(aggregate_metrics['tokens_per_sec'][-10:])).item():.2f}"
    )
    print(
        f"Average memory bandwidth of last 10 iterations: {torch.mean(torch.tensor(aggregate_metrics['memory_bw'][-10:])).item():.2f}"
    )
    print(
        f"Std. dev. memory bandwidth of last 10 iterations: {torch.std(torch.tensor(aggregate_metrics['memory_bw'][-10:])).item():.2f}"
    )
