...
def get_args():
    ...
    parser.add_argument(
        "--ignore-eos",
        action="store_true",
        help="Ignore EOS token in output generation.",
    )
    parser.add_argument(
        "--quantization", type=str, default=None, help="Quantization to use"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--warmup-duration", type=int, default=0, help="Warmup duration in minutes"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum number of sequences per vLLM iteration",
    )
    parser.add_argument(
        "--max-num-batched-tokens",
        type=int,
        default=None,
        help="Maximum number of batched tokens per vLLM iteration",
    )
    parser.add_argument(
        "--gpu-memory-utilization",
        type=float,
        default=None,
        help="The fraction of GPU memory to be used for the model executor in vLLM",
    )
    parser.add_argument(
        "--enforce-eager",
        type=bool,
        default=None,
        help="Always use eager-mode PyTorch. If False, will use eager mode and CUDA graph in hybrid for maximal performance and flexibility.",
    )
    parser.add_argument(
        "--block-size",
        type=int,
        default=None,
        help="Token block size for contiguous chunks of tokens for vLLM",
    )
    parser.add_argument(
        "--swap-space",
        type=int,
        default=None,
        help="CPU swap space size (GiB) per GPU for vLLM",
    )
    parser.add_argument(
        "--disable-custom-all-reduce",
        type=bool,
        default=None,
        help="Disable the custom all-reduce kernel in vLLM and fall back to NCCL",
    )
    parser.add_argument(
        "--max-seq-len-to-capture",
        type=int,
        default=None,
        help="Maximum sequence length covered by CUDA graphs. When a sequence has context length larger than this, we fall back to eager mode",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If set, the prefill requests can be chunked based on the max_num_batched_tokens",
    )
    parser.add_argument(
        "--sorting",
        type=str,
        default=None,
        choices=["ascending", "descending", "lexicographic", "skip"],
        help="Sorting method applied to samples",
    )
...
class DefaultEngineInput:
    ...
    enable_prefix_caching: bool = (
        False  # Setting to True adds minor perf gain for Offline
    )
...
def setup_llm_kwargs(args):
    ...
    llm_config.block_size = (
        args.block_size if args.block_size else llm_config.block_size
    )
    llm_config.swap_space = (
        args.swap_space if args.swap_space else llm_config.swap_space
    )
    llm_config.gpu_memory_utilization = (
        args.gpu_memory_utilization
        if args.gpu_memory_utilization
        else llm_config.gpu_memory_utilization
    )
    llm_config.max_seq_len_to_capture = (
        args.max_seq_len_to_capture
        if args.max_seq_len_to_capture
        else llm_config.max_seq_len_to_capture
    )
    llm_config.enforce_eager = (
        args.enforce_eager if args.enforce_eager else llm_config.enforce_eager
    )
    llm_config.disable_custom_all_reduce = (
        args.disable_custom_all_reduce
        if args.disable_custom_all_reduce
        else llm_config.disable_custom_all_reduce
    )
    llm_config.max_num_batched_tokens = (
        args.max_num_batched_tokens
        if args.max_num_batched_tokens
        else llm_config.max_num_batched_tokens
    )
    llm_config.max_num_seqs = (
        args.max_num_seqs if args.max_num_seqs else llm_config.max_num_seqs
    )
    llm_config.kv_cache_dtype = (
        args.kv_cache_dtype if args.kv_cache_dtype else llm_config.kv_cache_dtype
    )
    llm_config.enable_prefix_caching = (
        args.enable_prefix_caching
        if args.enable_prefix_caching
        else llm_config.enable_prefix_caching
    )
    llm_config.enable_chunked_prefill = (
        args.enable_chunked_prefill
        if args.enable_chunked_prefill
        else llm_config.enable_chunked_prefill
    )
