...
class SyncServerSUT(SUT):
    ...
    def send_samples(self, samples):
        items = [
            (str(sample.id), self.data_object.input_ids[sample.index])
    ...
    def send_outputs(self, qdata_out, is_first_token=False):
        ...
        while True:
            ...
            (
                self.post_proc_first(response)
                if is_first_token
                else self.post_proc(response)
            )
