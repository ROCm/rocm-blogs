...
def process_open_orca(dataset_name, device, tokenizer, seqlen, sample_len):
...
def get_calib_dataloader_to_list(
    ...
    if dataset_name == "pileval_for_awq_benchmark":
        ...
        samples = process_open_orca(
            dataset_name, device, tokenizer, seqlen, num_calib_data
        )
        # samples = [torch.tensor(s, dtype=torch.int32) for s in samples]
        # samples = torch.cat(tensor_samples)
    else:
        raise NotImplementedError

    calib_dataloader: DataLoader[List[Dict[str, torch.Tensor]]] = DataLoader(
        samples, batch_size=None, shuffle=False
    )  # type: ignore
...
def get_calib_dataloader_to_dict(
    ...
    calib_dataloader = DataLoader(
        tokenized_datasets, batch_size=batch_size, collate_fn=my_collate_fn
    )

    return calib_dataloader


def get_calib_dataloader(dataset_name: str, **kwargs: Any) -> Union[
    DataLoader[torch.Tensor],
    DataLoader[List[Dict[str, torch.Tensor]]],
    DataLoader[Dict[str, torch.Tensor]],
]:
    if dataset_name in ["pileval", "cnn_dailymail"]:
        ...
        dataset_name in ["pileval_for_awq_benchmark", "wikitext_for_gptq_benchmark"]
        or "open_orca" in dataset_name
    ):
