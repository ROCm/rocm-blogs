...
from quark.torch.quantization.config.custom_config import (
    DEFAULT_AWQ_CONFIG, DEFAULT_FLOAT16_CONFIG, DEFAULT_GPTQ_CONFIG,
    DEFAULT_SMOOTH_QUANT_CONFIG, DEFAULT_W_FP8_A_FP8_OFP8_PER_TENSOR_CONFIG,
    DEFAULT_W_FP8_A_FP8_PER_TENSOR_CONFIG, DEFAULT_W_INT4_PER_CHANNEL_CONFIG,
    DEFAULT_W_INT4_PER_GROUP_SYM_CONFIG, DEFAULT_W_INT4_PER_TENSOR_CONFIG,
    ...
    DEFAULT_W_UINT4_PER_GROUP_CONFIG, FP8_PER_TENSOR_SPEC)
...
def get_config(args: argparse.Namespace, model_type: str) -> Config:
    ...
    if args.quant_scheme == "w_fp8_a_fp8":
        ...
        if args.quant_algo == "awq":
            ...
            assert quant_config.algo_config is not None and hasattr(
                quant_config.algo_config, "scaling_layers"
            )
            quant_config.algo_config.scaling_layers = algo_config_info["scaling_layers"]
            quant_config.algo_config.model_decoder_layers = algo_config_info[
                "model_decoder_layers"
            ]
            quant_config.algo_config.embedding_layers = algo_config_info[
                "embedding_layers"
            ]
            ...
            with open(algo_config_file, "r") as file:
                algo_config_info = json.load(file)
            assert quant_config.algo_config is not None and hasattr(
                quant_config.algo_config, "inside_layer_modules"
            )
            quant_config.algo_config.inside_layer_modules = algo_config_info[
    ...
    if args.quant_algo is not None and args.quant_scheme != "w_uint4_per_group_asym":
        raise ValueError(
            f"{args.quant_algo} onlyNone support quant_schema as w_uint4_per_group_asym"
        )
...
def get_tokenizer(
    ckpt_path: str, max_seq_len: int = 2048, model_type: Optional[str] = None
) -> AutoTokenizer:
...
def get_model(
    ckpt_path: str, data_type: str = "auto", device: str = "cuda"
...
def main(args: argparse.Namespace) -> None:
    ...
    tokenizer = get_tokenizer(
        args.model_dir, max_seq_len=args.seq_len, model_type=model_type
    )
    ...
    # 3. Define calibration dataloader(still need this step for weight only
    ...
    if args.model_export is not None:
        # If user want to export the quantized model, please freeze the
    ...
    if args.model_export == "vllm_adopted_safetensors":
        ...
        with torch.inference_mode():
            ...
            from quark.torch.export.config.custom_config import (
                DEFAULT_EXPORTER_CONFIG, EMPTY_EXPORTER_CONFIG)

            config = (
                EMPTY_EXPORTER_CONFIG
                if args.no_weight_matrix_merge
                else DEFAULT_EXPORTER_CONFIG
            )
            exporter = ModelExporter(config=config, export_dir=export_path)
            exporter.export_model_info(
                model, model_type, model_dtype, export_type="vllm-adopt"
            )
    ...
    # Export option 3: torch.compile
        ...
        # Note: The model after torch.compile may not be able to export to
        # other format
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seq_len", type=int, help="Sequence length of data", default=512
    )
    parser.add_argument("--skip_quantization", action="store_true")
    parser.add_argument("--skip_evaluation", action="store_true")
    parser.add_argument(
        "--eval_metric",
        help="Model evaluate metric",
        default="ppl",
        choices=["ppl", "rouge"],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Whether to merge weight matrix when dump vllm-adopt quantized model",
        action="store_true",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        choices=[None, "torch_compile", "onnx", "vllm_adopted_safetensors"],
    )
