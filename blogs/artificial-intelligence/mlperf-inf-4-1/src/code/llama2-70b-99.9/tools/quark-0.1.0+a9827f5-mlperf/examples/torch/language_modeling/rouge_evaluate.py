...
def prepare_openorca(
    dataset_path: str,
) -> Tuple[List[List[int]], List[int], List[List[int]], List[str]]:
    try:
    ...
    for idx in tqdm(range(0, samples_num, batch_size)):
        ...
        for q in range(batch_size):
            ...
            input_ids = torch.tensor(
                source_ids[idx + q], device=device, dtype=torch.int32
            ).unsqueeze(0)
            ...
            input_ids_tensor.append(
                pad(
                    input_ids,
                    (max_seq_len - input_length, 0, 0, 0),
                    value=tokenizer.pad_token_id,
                )
            )
            input_masks_tensor.append(
                pad(attention_mask, (max_seq_len - input_length, 0, 0, 0), value=0)
            )
