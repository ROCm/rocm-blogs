...
# torch._inductor.config.fx_graph_cache = True # Experimental feature to
# reduce compilation times, will be on by default in future
...
def main(
    ...
    start = -1 if compile == "none" else 0
    ...
    print(
        f"Average tokens/sec: {
            torch.mean(
                torch.tensor(
                    aggregate_metrics['tokens_per_sec'])).item():.2f}"
    )
