...
#
# In this blog, we demonstrate training a simple ResNet model for image classification on AMD GPUs using AMD ROCm^TM^ on the CIFAR10 dataset. Training a ResNet model on AMD GPUs is simple, requiring no additional work beyond installing ROCm and appropriate PyTorch libraries.
#
# ## Introduction
#
...
#
# * Permute the channel dimension to 'channel first' format, as required by `pytorch`
# * Convert to `float32`
...
# ## Summary
#
# In this blog, we showed how to use an AMD GPU to train a ResNet image classifier on the CIFAR10 dataset, achieving 73% accuracy in less than a minute! All of this runs seamlessly on AMD GPUs with ROCm. We can further improve performance by employing several techniques, such as LR schedulers, data augmentation, and more training epochs, which we will leave as an exercise to the reader.
#
# ## References
#
