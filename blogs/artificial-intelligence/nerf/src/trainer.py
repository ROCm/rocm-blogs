...
import numpy as np
import torch
from dataset import (NerfDataset, encode_dirs, encode_pts,
                     hierarchical_sampling, stratified_sampling)
...
def nerf_forward(rays_o, rays_d, coarse_model, fine_model=None, n_samples=64):
    ...
    outputs = {"z_vals_stratified": z_vals, "rgb_map_0": rgb_map}
    ##########################################################################
    # Fine model pass
    ##########################################################################
    ...
    # (W*H, n_samples*2, 3+1)
    raw = raw.reshape(-1, n_samples * 2, raw.shape[-1])
    # Perform differentiable volume rendering to re-synthesize the RGB image.
...
def main(iters=361, lr=5e-4):
    ...
    for iter in range(iters):
        coarse_model.train()
        for idx in lis:
            rays_o, rays_d, target_img = dataset[
                idx
            ]  # [100, 100, 3], [100, 100, 3], [100, 100, 3]
            rays_o, rays_d, target_img = (
                rays_o.to("cuda"),
                rays_d.to("cuda"),
                target_img.to("cuda"),
            )
            outputs = nerf_forward(
                rays_o.reshape(-1, 3), rays_d.reshape(-1, 3), coarse_model, fine_model
            )
        ...
        if iter % 100 == 0:
            coarse_model.eval()
            rays_o, rays_d, target_img = test_dataset[
                0
            ]  # [100, 100, 3], [100, 100, 3], [100, 100, 3]
            rays_o, rays_d, target_img = (
                rays_o.to("cuda"),
