...
class NerfDataset(torch.utils.data.Dataset):
    def __init__(self, s=0, e=100):
        ...
        # 4*4 Rotational matrix to change camera coordinates to NDC(Normalized
        # Device Coordinates)
        self.poses = torch.from_numpy(data["poses"])[s:e]
...
def encode(pts, num_freqs):
    freq = 2.0 ** torch.linspace(0, num_freqs - 1, num_freqs)
    encoded_pts = []
    for i in freq:
...
def encode_dirs(dirs, n_samples=64, L=4):
    ...
    dirs = dirs[..., None, :].expand(
        dirs.shape[:-1] + (n_samples, dirs.shape[-1])
    )  # (W,H,num_samples,3)
    # print(dirs.shape)
...
def sample_pdf(
    ...
    # Sample random weights uniformly and find their indices in the CDF
    # distribution
    u = torch.rand(
    ...
    matched_shape = list(inds_g.shape[:-1]) + [
        cdf.shape[-1]
...
def hierarchical_sampling(
    ...
    new_z_samples = sample_pdf(
        z_vals_mid, weights[..., 1:-1], n_samples_hierarchical, perturb=perturb
    )
