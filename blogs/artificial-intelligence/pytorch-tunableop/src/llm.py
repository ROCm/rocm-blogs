...
def main(tune):
    ...
    # using torch. This is often less cumbersome than passing the environment
    # variables each time
    if tune:
        print("Tuning enabled")
        os.environ["PYTORCH_TUNABLEOP_ENABLED"] = "1"  # Enable tuning
        os.environ["PYTORCH_TUNABLEOP_FILENAME"] = (
            "src/llm_result.csv"  # Specify output file
        )
    ...
    # Set the model to use a static KV cache - see
    # https://huggingface.co/docs/transformers/main/en/llm_optims?static-kv=generation_config#static-kv-cache-and-torchcompile
    model.generation_config.cache_implementation = "static"

    # Tokenize our input.
    # Use padding with `pad_to_multiple_of` to minimize the number of GEMMs to tune
    # Larger values => Less GEMMs to tune, but more potential overhead for
    # shorter prompts
    inputs = tokenizer(
        prompt, return_tensors="pt", padding=True, pad_to_multiple_of=8
    ).to("cuda")
    ...
    n_tokens = seq_len - inputs["input_ids"].shape[1]
    ...
    for i in range(n_batches + n_warmup):
        ...
        model.generate(
            **inputs,
            ...
            # Force the model to generate exactly n_tokens before stopping
