...
def generate(idx, seed):
    ...
    # Pad the input sequence with a special padding token (e.g. 0, or 50256
    # which is the end of sequence token) so that they have the same length
    idx = jnp.array(
        [
            jnp.pad(
                jnp.array(sub_arr),
                (max_context_len + max_new_tokens - len(sub_arr), 0),
                mode="constant",
                constant_values=0,
            )
            for sub_arr in idx
    ...
    def body_fn(i, idx):
        ...
        if top_k is not None:
            ...
            )  # 0: value, 1: index; descending order
            logits = jnp.where(
