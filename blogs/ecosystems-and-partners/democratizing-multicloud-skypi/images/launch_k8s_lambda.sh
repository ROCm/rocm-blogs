...
echo -e "This script will deploy a Kubernetes cluster on the cloud and GPUs specified in cloud_k8s_lambda.yaml.\n"
...
sky launch -y -c ${CLUSTER_NAME} cloud_k8s_lambda.yaml
...
NVIDIA_LAMBDA_KUBECONFIG="$HOME/.kube/nvidia-h100-lambda-kubeconfig"
cp "$KUBECONFIG_FILE" "$NVIDIA_LAMBDA_KUBECONFIG"

echo "Copied LAMBDA kubeconfig file to ${NVIDIA_LAMBDA_KUBECONFIG}"
sleep 5
